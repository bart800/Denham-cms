// 🐩 Filevine FULL Data Extractor v2 — paste into Filevine browser console
// READ-ONLY: Does NOT modify any Filevine data
// Scrapes ALL sections: intake, case summary, negotiations, insurance, expenses, witnesses, experts
(async function() {
  const IDS = ["12363462","12363397","12363425","12363437","12363488","13098097","12363559","12363363","12363573","12653077","12451315","12363481","12363474","12363608","12363409","13102386","12363478","12363492","12846232","12363373","12363331","12363332","13263142","12363374","12363349","12363581","13107370","12802444","12363392","12653074","12650731","13329315","13098298","12363560","12363579","12401751","12846345","12803460","13354883","12363328","12453885","12363398","12722070","13079140","12461020","12543798","12363593","13136580","12653075","12978568","12572758","12454033","12363552","13050698","13354879","12735135","12363458","12978521","12363598","13459550","12363336","12363554","13089789","12363455","12847561","12795184","13083109","12363477","13403414","12363494","12363609","12363382","12380880","12363459","12363596","12693188","12543799","12363556","12363431","12847515","12982877","12363471","12363505","12375858","13098119","12363372","12363578","12363355","12401478","12363468","12363341","12981593","12363553","12363566","12363343","13081710","12382652","12710720","12363325","13407246","12363424","13403415","12363348","12804848","13098120","13102208","13076989","12363384","13089795","12363570","12795187","12872950","12363499","12363569","12363524","13374897","12847456","12384842","12363370","12363610","12379693","12454837","12363345","12363439","13251812","12363337","13221445","12459762","12363428","13468255","12363483","12977901","12363486","12363584","12648211","12363447","12363564","13098604","13104682","12363399","12453591","13506409","12380566","12363405","13631408","13245988","12363463","12543592","12653076","12401273","12795183","13462253","13407492","13138051","13476234","13406490","13403418","12363557","12363335","12363571","12543213","12363544","13255786","13640751","12942674","12363605","12363453","12363586","13054506","12785921","12733761","12574702","13360531","13418204","12363402","12983265","12671107","13227673","13245986","12363534","12363514","12672802","13576276","12363387","12668521","13485005","12363466","12363484","12668522","12843290","12945835","13571458","12363612","12363537","13103295","13218166","12363395","13577541","13238202","13103780","13263555","13593558","13360530","12572727","12379772","12363457","13263141","13485010","12363506","12363396","12543729","13595804","12722075","12363461","12363434","13654203","12363420","12380970","12363432","13438635","12384841","13102210","13502242","12363563","13447635","12545579","12363367","12722071","13459551","12363532","13098121","13646378","13098617","13403417","12363470","13221437","13229643","12363503","12363607","12363469","12544593","12403757","12368874","12363327","12363592","12363407","12363344","12363380","13219277","12363401","12573643","13645531","12744845","13098908","12363464","12382666","12363416","13485006","13672823","13476235","13089802","12545558","12795179","13505030","13374186","13672824","12415534","12669184","12363528","12403756","12877900","13412652","13654218","13485122","12650735","12363418","13571419","12363465","13251844","12403755","13098596","12363482","13595795","13652912","12944086","13593554","13571445","12670353","13674236","12363595","13677954","12877897","13594978","13680561","12453884","12461022","13595162","12877896","12363413","13572708","12363585","12384844","12363426","12363567","12363551"];

  const SECTIONS = [
    { name: 'intake', hash: 'intake33651' },
    { name: 'caseSummary', hash: 'casesummary33651' },
    { name: 'negotiations', hash: 'negotiations33651' },
    { name: 'insurance', hash: 'insurance33651' },
    { name: 'expenses', hash: 'expenses33651' },
    { name: 'witnesses', hash: 'witnessesdepos33651' },
    { name: 'experts', hash: 'experts33651' }
  ];

  const delay = ms => new Promise(r => setTimeout(r, ms));
  const results = [];
  const startTime = Date.now();

  function scrapeFields() {
    const data = {};
    document.querySelectorAll('input, select, textarea').forEach(el => {
      let label = '';
      const p = el.closest('[class*=field], .form-group, [class*=Field]');
      if (p) {
        const l = p.querySelector('label, [class*=label], [class*=Label]');
        if (l) label = l.textContent.trim();
      }
      if (!label) label = el.name || el.placeholder || el.getAttribute('aria-label') || '';
      if (label && el.value && el.value !== 'Unknown' && el.value !== 'on' && el.value !== '') {
        const key = label.replace(/[0-9\/]+\s*$/, '').trim();
        if (key && key.length < 100) data[key] = el.value;
      }
    });

    // Also grab any visible text in spans/divs that look like field values
    document.querySelectorAll('[class*=value], [class*=Value], [class*=display], [class*=Display]').forEach(el => {
      const p = el.closest('[class*=field], .form-group, [class*=Field], [class*=row], [class*=Row]');
      if (p) {
        const l = p.querySelector('label, [class*=label], [class*=Label]');
        if (l) {
          const key = l.textContent.trim();
          const val = el.textContent.trim();
          if (key && val && key.length < 100 && val.length < 500) {
            data[key] = val;
          }
        }
      }
    });

    return data;
  }

  function scrapeTable() {
    const rows = [];
    document.querySelectorAll('table tbody tr, [class*=table] [class*=row]:not([class*=header])').forEach(tr => {
      const cells = {};
      const headers = tr.closest('table')?.querySelectorAll('thead th') || [];
      tr.querySelectorAll('td').forEach((td, i) => {
        const header = headers[i]?.textContent?.trim() || ('col' + i);
        const val = td.textContent.trim();
        if (val) cells[header] = val;
      });
      if (Object.keys(cells).length > 0) rows.push(cells);
    });
    return rows;
  }

  console.log('🐩 Filevine Full Scraper v2 — ' + IDS.length + ' cases × ' + SECTIONS.length + ' sections');
  console.log('Estimated time: ~' + Math.round(IDS.length * SECTIONS.length * 2.5 / 60) + ' minutes');

  for (let i = 0; i < IDS.length; i++) {
    const pid = IDS[i];
    const elapsed = Math.round((Date.now() - startTime) / 1000);
    const eta = i > 0 ? Math.round((elapsed / i) * (IDS.length - i) / 60) : '?';
    console.log('[' + (i+1) + '/' + IDS.length + '] Scraping ' + pid + ' (elapsed: ' + Math.round(elapsed/60) + 'm, ETA: ' + eta + 'm)');

    const caseData = { projectId: pid, sections: {} };

    for (const section of SECTIONS) {
      try {
        location.hash = '#/project/' + pid + '/custom/' + section.hash;
        await delay(2000);

        const fields = scrapeFields();
        const tables = scrapeTable();

        if (Object.keys(fields).length > 0 || tables.length > 0) {
          caseData.sections[section.name] = { fields, tables };
        }

        // Grab project name from first section
        if (section === SECTIONS[0]) {
          const h1 = document.querySelector('h1');
          if (h1) caseData.projectName = h1.textContent.trim();
          const ph = document.querySelector('a[href^="tel:"]');
          if (ph) caseData.clientPhone = ph.textContent.trim();
          const em = document.querySelector('a[href^="mailto:"]');
          if (em) caseData.clientEmail = em.textContent.trim();
        }
      } catch(e) {
        console.error('  ✗ ' + section.name + ': ' + e.message);
        caseData.sections[section.name] = { error: e.message };
      }
    }

    const fieldCount = Object.values(caseData.sections).reduce((sum, s) => sum + Object.keys(s.fields || {}).length, 0);
    console.log('  ✓ ' + (caseData.projectName || pid) + ': ' + fieldCount + ' fields across ' + Object.keys(caseData.sections).length + ' sections');
    results.push(caseData);

    // Save progress every 25 cases
    if ((i + 1) % 25 === 0) {
      window._scrapedDataV2 = results;
      console.log('  💾 Progress saved to window._scrapedDataV2 (' + results.length + ' cases)');
    }

    // Breathing room every 10 cases
    if (i % 10 === 9) await delay(3000);
  }

  window._scrapedDataV2 = results;
  const totalTime = Math.round((Date.now() - startTime) / 1000 / 60);
  console.log('🎉 Done! ' + results.length + ' cases scraped in ' + totalTime + ' minutes');
  console.log('Access: window._scrapedDataV2');

  // Auto-download
  const blob = new Blob([JSON.stringify(results, null, 2)], {type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'filevine-full-data.json';
  a.click();
  console.log('📥 Downloaded filevine-full-data.json');
})();
